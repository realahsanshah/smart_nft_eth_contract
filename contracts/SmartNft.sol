...
contract SmartNft is ReentrancyGuard, Pausable, Ownable {
    struct Burner{
    ...
    mapping(address=>Burner) burnersMapping;
    ...
    function batchTransfer(address[] calldata tokenContracts, uint256[] calldata tokenIds, uint256[] calldata counts) public whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenContracts.length; i++) {
            ...
            if(burnersMapping[msg.sender].count==0){
                Burner memory burnerData = Burner({burnerAddress:msg.sender,count:1});
                burnersList.push(msg.sender);
                burnersMapping[msg.sender] = burnerData; 
            } 
            
            burnersMapping[msg.sender].count +=1;
        }
